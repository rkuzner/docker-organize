...
# evaluate if DATA_FOLDER is valid, if not use default
DATA_FOLDER="${DATA_FOLDER:-${DEFAULT_DATA_FOLDER}}"
if [ ! -d "${DATA_FOLDER}" ]; then
  log_message "Invalid DATA_FOLDER: ${DATA_FOLDER}"
  DATA_FOLDER="${DEFAULT_DATA_FOLDER}"
fi
log_message "Using DATA_FOLDER: ${DATA_FOLDER}"

# evaluate if DATA_FOLDER is valid, if not use default
ORGANIZE_CONFIG="${ORGANIZE_CONFIG:-${DEFAULT_ORGANIZE_CONFIG}}"
if [ ! -f "${ORGANIZE_CONFIG}" ]; then
  log_message "Invalid ORGANIZE_CONFIG: ${ORGANIZE_CONFIG}"
  ORGANIZE_CONFIG="${DEFAULT_ORGANIZE_CONFIG}"
fi
log_message "Using ORGANIZE_CONFIG: ${ORGANIZE_CONFIG}"

# evaluate if ORGANIZE_COMMAND is valid, if not use default
ORGANIZE_COMMAND="${ORGANIZE_COMMAND:-${DEFAULT_ORGANIZE_COMMAND}}"
if [ "${ORGANIZE_COMMAND}" != "sim" ] && [ "${ORGANIZE_COMMAND}" != "run" ]; then
  log_message "Invalid ORGANIZE_COMMAND: ${ORGANIZE_COMMAND}"
  ORGANIZE_COMMAND="${DEFAULT_ORGANIZE_COMMAND}"
fi
log_message "Using ORGANIZE_COMMAND: ${ORGANIZE_COMMAND}"
...
log_message "Append DATA_FOLDER info to ${toolConfigFileName} file"
echo 'DATA_FOLDER="'${DATA_FOLDER}'"' >> /${userHomeFolder}/${toolConfigFileName}

log_message "Append ORGANIZE_COMMAND to ${toolConfigFileName} file"
echo 'ORGANIZE_COMMAND="'${ORGANIZE_COMMAND}'"' >> /${userHomeFolder}/${toolConfigFileName}

log_message "Append ORGANIZE_CONFIG to ${toolConfigFileName} file"
echo 'ORGANIZE_CONFIG="'${ORGANIZE_CONFIG}'"' >> /${userHomeFolder}/${toolConfigFileName}
